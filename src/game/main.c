...
int main(int argc, char* argv[]) {
	...
	ODIN_Resource* meshTexture = Resources_getResource("helloworld.bmp");

	if (!Resources_loadResource("hellored.bmp")) {
		printf("Unable to load file 'hellored.bmp', error happend.\n");
	}

	ODIN_Resource* mesh2Texture = Resources_getResource("hellored.bmp");
	...
	while (!quit) {
		...
		Textures_bindTexture((ODIN_Texture*)meshTexture->buffer, GL_TEXTURE0);
		...
		//mat4_str(model2, buf);
		//printf("model2: %s\n", buf);
		Textures_bindTexture((ODIN_Texture*)mesh2Texture->buffer, GL_TEXTURE0);
		...
		// Unbind the textures
		Textures_unbindTexture(GL_TEXTURE0);
	...
	Resources_releaseResource(meshTexture);
